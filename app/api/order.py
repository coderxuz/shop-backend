...
async def cancel(order_id: int, request: Request, db: Session = Depends(get_db)):
    payload = verify_token(request)
    current_user = payload["sub"]
    db_user = db.query(User).filter(User.email == current_user).first()
    if not db_user:
    ...
    if db_user.id != order.customer_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="it is not your order"
        )
...
@router.get("/{order_id}",response_model=OrderGet, dependencies=[Depends(oauth2_scheme)])
async def show_order(order_id: int, request: Request, db: Session = Depends(get_db)):
    payload = verify_token(request)
    current_user = payload["sub"]
    db_user = db.query(User).filter(User.email == current_user).first()
    if not db_user:
